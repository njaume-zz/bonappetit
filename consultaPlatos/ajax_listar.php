...
$sql = "SELECT factura_detalles.demora_preparacion, tipo_recetas.pasa_cocina, factura_maestros.id, IF(factura_detalles.`fecha_alta` IS NULL, factura_maestros.`fecha_y_hora`, factura_detalles.`fecha_alta`) as fecha_alta, factura_detalles.`cantidad`,  
            ...
            if($tiempo>$tiempo_prep) $ver_estado = RestarHoras($tiempo_prep,$tiempo); else $ver_estado = "NO";                        
                ...
                ?>
                    ...
                    <tr  id="tr_<?php echo $rs_inf['id']?>" >
                            ...
                            <td><?php if($rs_inf['pasa_cocina']==1)  {
                                        echo $rs_inf['hora_cocina'];                                 
                            }   else echo ""; ?></td>  
                            <td><?php echo $rs_inf['hora_entrega']?></td>                     
                            <td><?php if($rs_inf['id_tipo_plato']<>4)
                                echo $ver_estado ?></td>                                    
                </tr>
