		...
		if(!empty($this->maestro_id)) {
			...
			if(($field->name != 'id') AND ($field->name != 'usuario_id')){
				...
				if(substr($field->name, -3) == '_id'){
					...
					$page_m .= '<td align="right"  valign="top" width="200"><strong>'.$this->build_friendly_names($field->name).':</strong></td><td>'.$valor_campo.'</td>';
					...
					$page_m .= '<td align="right"  valign="top" width="200"><strong>'.$this->build_friendly_names($field->name).':</strong></td><td>'.$row[$j].'</td>';
					...
					$page_m .= '<td align="right"  valign="top" width="200"><strong>'.$this->build_friendly_names($field->name).':</strong></td><td>'.$value_field.'</td>';
		...
		while($i < mysql_num_fields($selectFields)){
			...
			if($column->name != 'id'){
				...
				elseif(($counthidden <= $totalhidden-1) AND ($this->ocultos[$counthidden] == $column->name)){
					...
					$page .= '<input type="hidden" name="'.$column->name.'" id="'.$column->name.'" value="'.$diayhora.'"/><img src="images/helpicon.gif" title="'.$comment.'" alt="'.$comment.'">';
...
//				else { $clase = ':required'; }
				...
				elseif((substr($field->name, -3) == '_id') AND ($field->name != 'usuario_id')){
