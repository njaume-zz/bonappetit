...
// configuracion
	...
	var $db_name = 'ficuscarica';

  /* 
	var $db_host = 'localhost';
	var $db_user = 'root';
	var $db_password = 'Me579YeR';
	var $db_name = 'gestruck';        
 */    
...
//	var $singular = array ('tractor','semi','auditado','activo','recomendado');	// las fk
//	var $plural = array ('vehiculos','vehiculos','si_nos','si_nos','si_nos'); //las tabas correspondientes a las fk anteriores
...
// fin configuracion
	...
	function Scaffold($mostrar, $table, $max_records = 100, $fields = array(), $ocultos=array(), $relacion=array(), $htmlsafe = true, $width = NULL){
	...
	*/
	function list_table($msg = NULL, $where = null){
		...
		if(!empty($this->maestro_id)) {$input_maestro = '<input type="hidden" name="maestro_id" value="'.$this->maestro_id.'">';
		...
		echo $total.' Registro/s encontrado/s. Secci&oacute;n: <strong>'.$this->build_friendly_names(strtoupper(substr($this->table,0,-1))).'</strong>. | '
			. '<form name="newrecord" id="newrecord" action="'.$_SERVER['PHP_SELF'].'" method="post" style="display:inline">'.$input_maestro.'<input type="hidden" name="variablecontrolposnavegacion" value="new"><a href="javascript:document.newrecord.submit()">Registro nuevo '.$mensaje_maestro.'</a></form><br /><br />';
		...
		while($array = mysql_fetch_array($select)){
			$page .= (!($count % 2) == 0) ? '<tr style="background:'.$this->row_even.';">' : '<tr style="background:'.$this->row_odd.';">';
                        ...
                        foreach($array as $column => $value){
                                ...
                                if(($column == 'parametros_vehiculo_id') OR ($column == 'parametros_chofer_id')) $parametro_id = $value; 
                                
                                //FIN EXCLUSIVO GESTRUCK
	...
	function new_row(){
		...
		while($i < mysql_num_fields($selectFields)){
			...
			if($column->name != 'id'){
				...
				elseif((substr($column->name, -3) == '_id') AND ($column->name != 'usuario_id')){
				...
				elseif(($counthidden <= $totalhidden-1) AND ($this->ocultos[$counthidden] == $column->name)){
					...
					$page .= '<td align="right"  valign="top" width="200"><strong>'.$this->build_friendly_names($column->name).':</strong></td><td><textarea name="'.$column->name.'" id="'.$column->name.'" rows="5" cols="40"></textarea><img src="images/helpicon.gif" title="'.$comment.'" alt="'.$comment.'"></td>';
				}elseif($column->type == 'timestamp'){
					$page .= '<input type="hidden" name="'.$column->name.'" id="'.$column->name.'" value="'.date("Y-m-d H:i:s").'"/><img src="images/helpicon.gif" title="'.$comment.'" alt="'.$comment.'">';
				}elseif($column->name == 'clave'){
					$page .= '<td align="right"  valign="top" width="200">(todo en mayusculas) <strong>'.$this->build_friendly_names($column->name).':</strong></td><td><input type="password" name="'.$column->name.'" id="'.$column->name.'" /><img src="images/helpicon.gif" title="'.$comment.'" alt="'.$comment.'"></td>';
				}elseif($column->type == 'date'){
					$page .= '<td align="right"  valign="top" width="200"><strong>'.$this->build_friendly_names($column->name).':</strong></td><td><input type="text" class="calendario" name="'.$column->name.'" id="'.$column->name.'" /><img src="images/helpicon.gif" title="'.$comment.'" alt="'.$comment.'"></td>';
				}elseif($column->type == 'time'){
					$page .= '<td align="right"  valign="top" width="200"><strong>'.$this->build_friendly_names($column->name).':</strong></td><td><input type="text" name="'.$column->name.'" id="'.$column->name.'" class="timepickr" /><img src="images/helpicon.gif" title="'.$comment.'" alt="'.$comment.'"></td>';                           
                                }elseif($column->name == 'usuario_id'){
                                /*borrar ->*/   $page .= '<input type="hidden" name="'.$column->name.'" id="'.$column->name.'" value="'.$this->usuario_id.'" />';
					...
					$page .= '<td width="200" align="right" ><strong>'.$this->build_friendly_names($column->name).':</strong></td><td><input class="'.$clase.'" type="text" name="'.$column->name.'" id="'.$column->name.'" /><img src="images/helpicon.gif" title="'.$comment.'" alt="'.$comment.'">';
					}else{
				...
				*/}else{
					...
					$page .= '<td align="right"  width="200"><strong>'.$this->build_friendly_names($column->name).':</strong></td><td><input class="'.$clase.'" type="text" name="'.$column->name.'" id="'.$column->name.'" value="" size="35" /><img src="images/helpicon.gif" title="'.$comment.'" alt="'.$comment.'"></td>';
				}
			}
			$i++;
		}
...
//				else { $clase = ':required'; }
				...
				if((substr($field->name, -3) == '_id') AND ($field->name != 'usuario_id')){
		...
		$page .= '<tr><td>&nbsp;</td><td><input type="submit" value="Guardar" /></td></tr>'
				 . '</table>'
				 . '</form>'
				 . '<a href="'.$_SERVER['PHP_SELF'].'">Volver al listado</a>';
	...
	function build_foreign_key_title($field, $value) {
		...
		if (!empty($foreign['descripcion'])){
				//ESTO LO PODRÍA HACER CON LIGHTBOX JQUERY
                    $dd .='<form name="searchbar'.$foreign['id'].'" id="searchbar'.$foreign['id'].'" action="'.$foreignTable.'.php" method="post"  target="ventanaForm" onsubmit="window.open(\'\', \'ventanaForm\', \'width=1024,height=400,top=100,left=100,scrollbars=yes\')"><input type="hidden" name="variablecontrolposnavegacion" value="search"><input type="hidden" name="field" value="descripcion"><input type="hidden" name="compare" value="="><input type="hidden" name="searchterm" value="'.$foreign['descripcion'].'"><input type="submit" name="Search" value="'.$foreign['descripcion'].' - '.$foreign['id'].'" class="boton_relacion" />  
